...
In each terminal, exec into the `ai_sapiens` container first:
...
You can also read a service's log file directly instead of going through `cycloctl`. Exec into the `ai_sapiens` container and tail the file:
