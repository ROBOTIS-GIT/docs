...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '16px', margin: '20px 0', alignItems: 'start'}}>
  ...
  <div>
    ...
    <p style={{marginTop: '10px', fontSize: '14px', color: '#888', fontStyle: 'italic', textAlign: 'center'}}>GR00T 1.7 Manipulation</p>
  </div>
</div>
...
For **[AI Worker](/docs/systems/aiworker/iFntroduction)**, our **bimanual semi-humanoid** combines **dual 7-DoF arms**, a **swerve-drive mobile base**, RGBD cameras, and on-device **NVIDIA Jetson AGX Orin** compute. The challenge therefore shifts from dynamics to data: **NVIDIA Isaac GR00T 1.7** provides the pretrained robot foundation model, while **NVIDIA Cosmos Transfer** helps broaden the visual diversity of real-robot demonstrations through synthetic augmentation.
...
* How **NVIDIA Cosmos Transfer** can augment a small real-robot dataset and improve a fine-tuned **NVIDIA Isaac GR00T 1.7** manipulation policy.
...
* [Part 2 — AI Worker: manipulation](#part-2--ai-worker-manipulation)
  ...
  * [Fine-tuning NVIDIA Isaac GR00T 1.7](#fine-tuning-nvidia-isaac-groot-17)
...
<div style={{display: 'grid', gridTemplateColumns: '1fr', gap: '20px', margin: '24px 0'}}>
  ...
  <div>
    <div style={{overflow: 'hidden', borderRadius: '30px'}}>
      <img src="/img/systems/aisapiens/resources/technical_story/building_humanoid_skills/aiw_pipeline.png" alt="AI Worker manipulation pipeline: teleoperation, Cosmos Transfer, GR00T 1.7, evaluation" style={{width: '100%', height: 'auto', aspectRatio: '860 / 230', objectFit: 'contain', display: 'block'}} />
    </div>
    <p style={{marginTop: '10px', fontSize: '14px', color: '#888', fontStyle: 'italic', textAlign: 'center'}}>AI Worker — Teleoperation → Cosmos Transfer → GR00T 1.7 → Evaluation</p>
  </div>
...
| Wall-clock training time | ~4 hours (measured for the “Red Red Dance” motion)           |
...
First, we fine-tuned **NVIDIA Isaac GR00T 1.7** on real-robot demonstrations and evaluated the resulting policy on AI Worker.
...
### Fine-tuning NVIDIA Isaac GR00T 1.7

Our first manipulation experiment focused on adapting **NVIDIA Isaac GR00T 1.7** to the AI Worker embodiment and evaluating how the fine-tuned policy behaved on the real robot.

We collected **268 teleoperated episodes**, approximately three hours of real-robot demonstrations, and used them to fine-tune **NVIDIA Isaac GR00T 1.7**.

| Parameter                 | Value                                   |
| ------------------------- | --------------------------------------- |
| Real-robot demonstrations | 268 teleoperated episodes (~3 hours)    |
| Base model                | NVIDIA Isaac GR00T 1.7                  |
| Training hardware         | NVIDIA RTX PRO 6000                     |
| Training time             | ~4 hours                                |
| Inference hardware        | NVIDIA Jetson AGX Orin 32 GB, on-device |

*Table 4. GR00T 1.7 fine-tuning configuration for AI Worker.*

Fine-tuning adapts the pretrained GR00T 1.7 model to the **AI Worker embodiment, workspace, and manipulation task** represented in our demonstrations.
...
<div style={{margin: '24px 0'}}>
  ...
  <p style={{marginTop: '10px', fontSize: '14px', color: '#888', fontStyle: 'italic', textAlign: 'center'}}>Before augmentation — the fine-tuned GR00T 1.7 policy occasionally reaches toward the gripper's shadow instead of the target object.</p>
</div>
...
We then fine-tuned GR00T 1.7 again using the expanded dataset.
...
<div style={{margin: '24px 0'}}>
  ...
  <p style={{marginTop: '10px', fontSize: '14px', color: '#888', fontStyle: 'italic', textAlign: 'center'}}>After Cosmos augmentation and GR00T 1.7 fine-tuning — improved target attention and re-approach behavior after a missed grasp.</p>
</div>
...
<strong>268 real demonstrations + 300 Cosmos-augmented demonstrations = 568 training samples</strong>
  ...
  <div style={{marginTop: '8px'}}>
    Real-robot data → GR00T 1.7 fine-tuning → real-world failure → Cosmos augmentation → retraining → improved robustness
  </div>

</div>

For this experiment, **NVIDIA Isaac GR00T 1.7** provided the pretrained foundation we adapted to the AI Worker embodiment and task, while **NVIDIA Cosmos Transfer** provided a way to broaden visual diversity without collecting another 300 demonstrations on the physical robot.
...
<div style={{margin: '24px 0'}}>
  <div style={{overflow: 'hidden', borderRadius: '16px'}}>
    <video controls autoPlay muted loop playsInline preload="metadata" style={{width: '100%', height: 'auto', display: 'block', aspectRatio: '1202 / 628', objectFit: 'contain'}}>
...
For **manipulation**, NVIDIA Isaac GR00T 1.7 gives us a strong pretrained foundation, while NVIDIA Cosmos Transfer helps us extract more value from expensive real-robot demonstrations by broadening their visual diversity.
...
* [Newton](https://developer.nvidia.com/newton-physics) — physics engine used to improve Sim2Real transfer for whole-body motion
