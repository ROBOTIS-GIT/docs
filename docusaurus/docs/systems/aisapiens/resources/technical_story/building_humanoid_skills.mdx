...
For **[AI Sapiens](/docs/systems/aisapiens/introduction)**, our 23-DoF bipedal humanoid, **NVIDIA Isaac Lab** provides the large-scale simulation and reinforcement-learning environment needed to train dynamic whole-body skills before deploying them to real hardware.

For **[AI Worker](/docs/systems/aiworker/introduction)**, our bimanual semi-humanoid, the challenge shifts from dynamics to data. **NVIDIA Isaac GR00T N1.7** provides the pretrained robot foundation model, while **NVIDIA Cosmos Transfer** helps broaden the visual diversity of real-robot demonstrations through synthetic augmentation.
...
**AI Sapiens** is a 23-DoF bipedal humanoid designed for whole-body motion. Every skill must be executed while maintaining dynamic balance, making simulation fidelity, contact dynamics, and real-time control critical to the learning pipeline.

**AI Worker** is a bimanual semi-humanoid on a wheeled base designed for manipulation. Here, whole-body balance is no longer the dominant bottleneck. The challenge shifts toward perception, data coverage, and generalization across real-world interactions.
...
<div style={{display: 'grid', gridTemplateColumns: '1fr', gap: '20px', margin: '24px 0'}}>
  <div>
    <div style={{overflow: 'hidden', borderRadius: '30px'}}>
      <img src="/img/systems/aisapiens/resources/technical_story/building_humanoid_skills/ais_pipeline.png" alt="AI Sapiens whole-body motion pipeline: Kimodo and GEM-X, SOMA-Retargeter, BeyondMimic, Sim2Real" style={{width: '100%', height: 'auto', aspectRatio: '860 / 230', objectFit: 'contain', display: 'block'}} />
    </div>
    <p style={{marginTop: '10px', fontSize: '14px', color: '#888', fontStyle: 'italic', textAlign: 'center'}}>AI Sapiens — Kimodo / GEM-X → SOMA-Retargeter → BeyondMimic → Sim2Real</p>
  </div>
...
**1. Motion input.** Multiple sources can feed the same pipeline.

* **Video / [GEM-X](/docs/systems/aisapiens/motion_generation/gem-x)** recovers 3D human motion from unconstrained video.
* **Text prompt / [Kimodo](/docs/systems/aisapiens/motion_generation_retargeting/kimodo)** generates kinematic motion from natural-language descriptions.
