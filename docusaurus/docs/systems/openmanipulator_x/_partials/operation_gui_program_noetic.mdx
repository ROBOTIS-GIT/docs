:::info

**NOTE**:
- This instructions are supposed to be running on PC with ROS packages installed in. Please run the instructions below on your PC ROS packages installed in.
- Make sure to run the [OpenMANIPULATOR-X controller](/docs/systems/openmanipulator_x/ros_controller/ros_controller_package#launch-controller) instructions before running the GUI Program.

:::


1. Launch the GUI program. The [OpenMANIPULATOR-X Controller](/docs/systems/openmanipulator_x/ros_controller/ros_controller_package#launch-controller) must be running on another terminal.
```bash
$ roslaunch open_manipulator_control_gui open_manipulator_control_gui.launch
```

2. Click the `Timer Start` button.  
![](/img/systems/openmanipulator_x/ros/OpenManipulator_GUI.webp)  

3. Check the status of OpenMANIPULATOR-X.  
![](/img/systems/openmanipulator_x/ros/ros_operation/OpenManipulator_GUI2.webp)  

4. To manipulate OpenMANIPULATOR-X in the [joint space], enter the joint angles and total time for the trajectory. Then click the `send` button to start the motion.  
![](/img/systems/openmanipulator_x/ros/ros_operation/OpenManipulator_GUI3.webp)  

5. To manipulate OpenMANIPULATOR-X in the [task space], enter the kinematics pose of the OpenMANIPULATOR-X end-effector(tool) in the [task space] and the total time for the trajectory. Then click the `send` button to start the motion.  
![](/img/systems/openmanipulator_x/ros/ros_operation/OpenManipulator_GUI4.webp)  

6. To create a drawing trajectory with OpenMANIPULATOR-X, choose the drawing trajectory type(line, circle, rhombus, heart). And enter the parameters according to the drawing trajectory type and the total time of the drawing trajectory. Then click the `send` button to start drawing.  
![](/img/systems/openmanipulator_x/ros/ros_operation/OpenManipulator_GUI5.webp)  

[task space]: /docs/systems/openmanipulator_x/msgs/open_manipulator_coordinates
[joint space]: /docs/systems/openmanipulator_x/msgs/open_manipulator_coordinates
