...
Please refer to the [SIFT documentation](https://docs.opencv.org/master/da/df5/tutorial_py_sift_intro.html) for more information.


1. Open a new terminal and launch the Autorace Gazebo simulation. Launch `roscore` with the **roslaunch** command.
```bash
$ roslaunch turtlebot3_gazebo turtlebot3_autorace_2020.launch
```

2. Open a new terminal and launch the teleoperation node. Drive the TurtleBot3 along the lane and stop where traffic signs can be clearly seen by the camera.
```bash
...
5. Capture each traffic sign from the `rqt_image_view` and crop unnecessary part of image. For the best performance, it is recommended to use original traffic sign images as seen on the track.
...
9. Select `/detect/image_traffic_sign/compressed` topic from the drop down list. A screen will display the result of traffic sign detection.
    ...
    ![](/img/systems/turtlebot3/autonomous_driving/autonomous_driving/noetic_detect_intersection.webp)

    > Detecting the Intersection sign when `mission:=intersection`

    ![](/img/systems/turtlebot3/autonomous_driving/autonomous_driving/noetic_detect_left.webp)

    > Detecting the Left sign when `mission:=intersection`

    ![](/img/systems/turtlebot3/autonomous_driving/autonomous_driving/noetic_detect_right.webp)

    > Detecting the Right sign when `mission:=intersection`

    ![](/img/systems/turtlebot3/autonomous_driving/autonomous_driving/noetic_detect_construction.webp)

    > Detecting the Construction sign when `mission:=construction`

    ![](/img/systems/turtlebot3/autonomous_driving/autonomous_driving/noetic_detect_parking.webp)

    > Detecting the Parking sign when `mission:=parking`

    ![](/img/systems/turtlebot3/autonomous_driving/autonomous_driving/noetic_detect_level_crossing.webp)

    > Detecting the Level Crossing sign when `mission:=level_crossing`

    ![](/img/systems/turtlebot3/autonomous_driving/autonomous_driving/noetic_detect_tunnel.webp)
