...
This guide shows how to run the Cyclo Control from [`cyclo_control`](https://github.com/ROBOTIS-GIT/cyclo_control) on OMY.
...
![omy_motion_controller](/img/systems/omy/advanced_features/cyclo_control/omy_motion_controller.png)
...
![omy_collision](/img/systems/omy/advanced_features/cyclo_control/omy_collision.gif)
...
![default_vs_movel_omy](/img/systems/omy/advanced_features/cyclo_control/default_vs_movel_omy.gif)
...
:::info
The default commands in this guide are written for `OMY_F3M`.

When running with the `OMY_3M` model, replace `omy_f3m` with `omy_3m` in the bringup command and override the robot model path if needed.
:::

## Bring Up OMY

Start the OMY follower bringup first. Run this on the `ROBOT PC`:
...
- Publish a `MoveL` command directly to `/omy_movel_controller/movel`.

![marker_control](/img/systems/omy/advanced_features/cyclo_control/omy_interactive_marker_real.gif)
...
![movel_control](/img/systems/omy/advanced_features/cyclo_control/omy_movel.gif)
...
![movej_control](/img/systems/omy/advanced_features/cyclo_control/omy_movej.gif)
...
- `/joint_states`: Measured OMY joint state required by all controller modes
- `/leader/joint_trajectory`: Output joint trajectory command published by the controllers

### MoveL

- `/omy_movel_controller/movel`: Cartesian `MoveL` input for the default controller
- `/omy_movel_controller/current_pose`: Current end-effector pose published by the default controller
- `/omy_movel_controller/controller_error`: Controller error status for the default controller

### MoveJ

- `/omy_movej_controller/movej`: Joint-space command topic for `movej`
...
- If the `movel` controller does not move, check the terminal log to see whether `/joint_states` is updating and whether `MoveL` commands are arriving on `/omy_movel_controller/movel`.
- If `movej` does not respond, confirm the published joint names match the OMY model joints.

## Controller Parameters

The main parameters live in `cyclo_motion_controller_ros/config/omy_config.yaml`. The file is divided by controller name, so you usually tune only the block that matches the controller mode you are running.
