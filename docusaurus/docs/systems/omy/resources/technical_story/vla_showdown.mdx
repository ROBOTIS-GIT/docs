...
Two driver packages are used: `usb_cam` for the two USB cameras and `realsense2_camera` for the wrist RealSense. Bring each camera up on the robot in its own terminal.
...
Each model was trained with its authors' most-recommended config (hardware was chosen based on size and availability). One thing changed was VLA-JEPA's published learning rate ($1\times10^{-4}$) diverged into NaNs on two
runs, so we used $3\times10^{-5}$.

| | GR00T N1.7 | MolmoAct2 | VLA-JEPA | FastWAM |
|---|---|---|---|---|
| Regime | partial (head + top-4 LLM) | full FT | head-only (Qwen frozen) | full FT |
| Final ckpt | 057643 | 20000 | 230574 | 165000 |
...
|---|---|---|---|
| **GR00T N1.7** | **0.017** | **7.5 mm** | ckpt 057643, held-out |
| MolmoAct2 | 0.019 | 9.9 mm | ckpt 12000, held-out |
| FastWAM | 0.034 | 24.8 mm | ckpt 165000, held-out |
...
```bash
# Run it: more denoising (flow-integration) steps, in chunk mode (the source fix).
...
first-try success over **10 trials per plate** on hardware.
...
```bash
# example, our best run: GR00T N1.7, ckpt 057643
