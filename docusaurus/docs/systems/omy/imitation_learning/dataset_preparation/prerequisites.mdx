...
Refer to the [External PC Control](/docs/systems/omy/specifications/zenoh_communication#external-pc-control) section in [Zenoh Communication](/docs/systems/omy/specifications/zenoh_communication) for the network and `ZENOH_CONFIG_OVERRIDE` settings.
...
:::info
The `ZENOH_CONFIG_OVERRIDE` environment variable must also be correctly set inside the **physical_ai_server Docker container** according to the [External PC Control](/docs/systems/omy/specifications/zenoh_communication#external-pc-control) configuration. Otherwise, ROS 2 communication may not work.
:::
