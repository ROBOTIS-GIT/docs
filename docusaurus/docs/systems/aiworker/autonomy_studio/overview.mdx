...
The underlying task logic is presented as reusable **Steps**, so you can focus on what the robot should do and the order in which it should happen. Choose a workspace mainly by whether the robot will work at one location or travel between several work locations:
...
Map and Mission names accept letters, numbers, `.`, `_`, and `-` and are limited to 128 characters. Use short, descriptive names such as `lab-east` and `morning-patrol`.
