...
Getting a trustworthy $P_{camera}$ in the first place — since CenterPose's own 3D point can't be trusted — is most of the work below. Its distance (depth) estimate has too much error to grasp with, so that value is discarded — only its 2D pixel direction is trusted, and the real depth comes from the ZED camera's own depth image instead:
...
**Step 2 — 180° front/back flip correction.** CenterPose occasionally reports an object rotated 180° from its true orientation. That mistake always shows up as an unusually large *unsigned* angle from the reference, so it's simple to detect and correct before trusting the angle:
