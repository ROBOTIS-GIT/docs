# Introduction
![](/img/parts/interface/dynamixel_shield/with_arduino.webp)

DYNAMIXEL Shield  was created to use [RC-100] and DYNAMIXEL on arduino board. We provide dynamixel library for DYNAMIXEL Shield, it can help you to use DYNAMIXEL easily.


:::warning

**This product does not contain Arduino Board. Arduino Board should be purchased separately.**

:::


# Specifications
|       Item        |                 Details                 |
|:-----------------:|:---------------------------------------:|
| Operating Voltage |  5 V (XL-330) ~ 24 V (PRO / X Series)   |
|  Maximum Current  | 1 A(Arduino), 10 A (Terminal Connector) |


:::danger

![](/img/icon_warning.webp)  
**DANGER**  
(Ignoring these warnings may cause serious injury or death)

-  Never place items containing water, flammables/open flames, or solvents near the product. 
- Never place fingers, arms, toes, and other body parts near product during operation.
- Cease operation and remove power from the product if the product begins to emit strange odors, noises, or smoke.
- Keep product out of reach of children.
- Check input polarity before installing or energizing wiring or cables.

:::


:::warning

![](/img/icon_warning.webp)  
**CAUTION**  
(Ignoring these warnings may cause mild injury or damage to the product)
 
- Do not insert blades or other sharp objects during product operation.

:::


:::warning

![](/img/icon_warning.webp)  
**ATTENTION**  
(Ignoring these warnings may cause minor injury or damage to the product)
- Do not disassemble or modify the product.
- Do not drop the product or apply strong impacts.
- Do not connect or disconnect DYNAMIXEL cables while power is being supplied.

:::


## Supported DYNAMIXEL
| DYNAMIXEL Series |                                                              |                                  |                                 |                                |                 |
|:-----------------|:-------------------------------------------------------------|:---------------------------------|:--------------------------------|:-------------------------------|:----------------|
| **AX**           | [AX-12W]                                                     | [AX-12+/12A]                     | [AX-18F/18A]                    |                                |                 |
| **RX** `1`       | [RX-10]                                                      | [RX-24F]                         | [RX-28]                         | [RX-64]                        |                 |
| **DX** `1`       | [DX-113]                                                     | [DX-116]                         | [DX-117]                        |                                |                 |
| **EX** `1`       | [EX-106+]                                                    |                                  |                                 |                                |                 |
| **MX**           | [MX-12W]                                                     | [MX-28], [MX-28(2.0)]            | [MX-64], [MX-64(2.0)]           | [MX-106], [MX-106(2.0)]        |                 |
| **XL**           | [XL320]                                                      | [XL430-W250]                     | [XL330-M077] <br /> [XL330-M288] | [2XL430-W250]                  |                 |
| **XC**           | [XC330-T288]<br />[XC330-T181]<br />[XC330-M288]<br />[XC330-M181] | [XC430-W150]<br /> [XC430-W240]<br />[XC430-T150BB]<br /> [XC430-T240BB]   | [2XC430-W250]                   |                                |                 |
| **XM**           | [XM430-W210]                                                 | [XM430-W350]                     | [XM540-W150]                    | [XM540-W270]                   |                 |
| **XH**           | [XH430-W210]<br /> [XH430-W350]                               | [XH430-V210]<br /> [XH430-V350]   | [XH540-W150]<br /> [XH540-W270]  | [XH540-V150]<br /> [XH540-V270] |                 |
| **XD**           | [XD430-T210-R]<br />[XD430-T350-R]                             | [XD540-T150-R]<br />[XD540-T270-R] |                                 |                                |                 |
| **XW**           | [XW430-T333-R] <br />[XW430-T200-R]                            | [XW540-T140-R]<br />[XW540-T260-R] |                                 |                                |                 |
| **PRO H**        | [H42-20-S300-R]                                              | [H54-100-S500-R]                 | [H54-200-S500-R]                |                                |                 |
| **PRO M**        | [M42-10-S260-R]                                              | [M54-40-S250-R]                  | [M54-60-S250-R]                 |                                |                 |
| **PRO L** `1`    | [L42-10-S300-R]                                              | [L54-30-S500-R]                  | [L54-30-S400-R]                 | [L54-50-S500-R]                | [L54-50-S290-R] |
| **PRO H(A)**     | [H42-20-S300-R(A)]                                           | [H54-100-S500-R(A)]              | [H54-200-S500-R(A)]             |                                |                 |
| **PRO M(A)**     | [M42-10-S260-R(A)]                                           | [M54-40-S250-R(A)]               | [M54-60-S250-R(A)]              |                                |                 |
| **PH**           | [PH42-020-S300-R]                                            | [PH54-100-S500-R]                | [PH54-200-S500-R]               |                                |                 |
| **PM**           | [PM54-060-S250-R]                                            | [PM54-040-S250-R]                | [PM42-010-S260-R]               |                                |                 |

`1` RX, DX, EX series are by default disabled and require [config.h](https://github.com/ROBOTIS-GIT/Dynamixel2Arduino/blob/master/src/utility/config.h#L6) modification in DYNAMIXEL2Arduino to be used.

# Layout
![](/img/parts/interface/dynamixel_shield/pinmap.webp)


:::warning

The DYNAMIXEL Shield has the same pin position as Aruduino UNO. To find the pinout diagram, see [Arduino Offical page](https://www.arduino.cc/en/Main/Products).

:::


| Pin No. |  Pin Name   |        Description        |
|:-------:|:-----------:|:-------------------------:|
|    0    | HW UART RX  |          DXL_RX           |
|    1    | HW UART TX  |          DXL_TX           |
|    2    | HW UART DIR |    DXL_DIR(DXL_TX_EN)     |
|    7    | SW UART RX  | `Caution2` SoftwareSerial |
|    8    | SW UART TX  | `Caution2` SoftwareSerial |

|      Item      |                 Description                 |          Note           |
|:--------------:|:-------------------------------------------:|:-----------------------:|
| DYNAMIXEL Port |           TTL, TTL(XL-320), RS485           |       `Caution3`        |
|  Power Switch  |   Power SW (DYNAMIXEL Port Power Switch)    |            -            |
|  UART Switch   | UART SW (Upload or DYNAMIXEL Select Switch) |       `Caution1`        |
|   Jumper Cap   |           Power Source Selection            | Read 'Connecting Power' |

-	DXL_RX (0), DXL_TX (1) : Hardware serial port to communicate with DYNAMIXEL.
- DXL_DIR(2) : Flow control of DYNAMIXEL packet.
- The hardware serial port is used to communicate with DYNAMIXEL, therefore, [RC-100] or [LN-101] has to be connected to the software serial communication port(SW TX/RX) via Arduino pin 7 and 8.


:::warning

`Caution1` When uploading firmware using USB port, you should switch the UART SW(SW_2) to Upload mode. When you select the UART SW (SW_2) to DYNAMIXEL mode, you can use DYNAMIXEL but USB port.  
`Caution2` If you are using a board that does not support SoftwareSerial(like SAMD, etc..), you cannot use pins 7 and 8 for UART purposes.  
`Caution3` TTL, TTL (XL-320) and RS485 connectors are all connected in parallel in one serial.

:::


## Use of Serial Monitor with DYNAMIXEL Shield
DYNAMIXELShield uses serial pins (0,1) which are the same pins as Arduino Uno / Mega. When using the serial monitor, it may cause unexpected issue with data in the board due to a port conflict.  

In order to prevent the board from the port conflict, be sure to read [How to Use Serial Monitor with DYNAMIXEL Shield](#how-to-use-serial-monitor-with-dynamixel-shield) carefully. 

### How to Use Serial Monitor with DYNAMIXEL Shield
Check the type of your arduino board, and select the either of listed solution to use Serial Monitor with DYNAMIXEL Shield. 

1. **[Recommanded] #define DEBUG_SERIAL**
- See #define DEBUG_SERIAL in the DynamixelShield Examples in Arduino.

2. **Use UART port of DYNAMIXEL Shield ( AVR 8 bit board including Uno / Mega boards)**
- AVR 8 bit boards, such as Uno, and Mega, can use the serial monitor by using DYNAMIXEL Shield's pins, which is read UART (See [Layout](#layout))


:::note

    **NOTE**: Be sure to use `SoftwareSerial` Library to use the serial port with DYNAMIXEL Shield and AVR 8 bit boards as the port is designed for the use of general pins. Preferred Communication Modules: BT-210, BT-410, LN-101.

:::


3. **Use USB Port**
- When you use other arduino boards except Uno and Mega, use the serial monitor via USB port.

4. **Use USB to Serial converter**
- You can use either SoftwareSerial or HardwareSerial ways. See [Arduino Reference page of Serial](https://www.arduino.cc/reference/en/language/functions/communication/serial/), and determine your board whether or not it support either of ways (SoftwareSerial or HardwareSerial).
 
5. **Use UART Port**
- DYNAMIXEL Shield contains UART pins: 7(RX), 8(TX). They are only compatible with AVR 8 bit board, such as Uno and Mega boards. In order to use this port, use other HardwareSerial pins instead of 7(RX), 8(TX). To determine whether boards have HardwareSerial, and its pin numbers see [Arduino Reference page of Communication](https://www.arduino.cc/reference/en/language/functions/communication/serial/). 

# Connecting Power
|                            Connect VIN                            |                          Disconnect VIN                           |
|:-----------------------------------------------------------------:|:-----------------------------------------------------------------:|
| ![](/img/parts/interface/dynamixel_shield/power_01.webp) | ![](/img/parts/interface/dynamixel_shield/power_02.webp) |

- The power input is divided as follows depending on whether the jumper cap is connected or not.
  1. Connect VIN : Power supply using with Arduino's SMPS connector
  2. Disconnect VIN : Power supply using DYNAMIXEL shield's power input connector. Can not use power from arduino.

- Since DYNAMIXEL power can not be connected to the USB power of the Arduino board, you need to connect the external power.
-	When using the power input connector, **be sure to check the polarity of the power supply**.

  ![](/img/parts/interface/dynamixel_shield/power_polarity.webp)


# Switches
![](/img/parts/interface/dynamixel_shield/switches.webp)

## Upload Switch
-	For DYNAMIXEL control, the hardware serial port of the arduino board is used. Because the downloading is done to the same port, the two ports may collide. Therefore, to download, use serial port switch to move to upload position and download.
- After the download is completed, the switch must be moved to DYNAMIXEL position for DYNAMIXEL control. If it is not moved, DYNAMIXEL will not work.

## DYNAMIXEL Power Switch
-	It is a switch that can turn on / off the power of DYNAMIXEL connector.


# DYNAMIXEL Shield Libraries
DYNAMIXEL2Arduino Library : [GitHub Repository][DYNAMIXEL2Arduino GitHub repository]  
DYNAMIXEL Shield Library : [GitHub Repository][DYNAMIXEL Shield GitHub repository]

## Features
- Compatible to Arduino boards
-	Support dynamixel protocol 1.0/2.0
- Up to 16 DYNAMIXEL's can be controlled (Typically, each motor(XL-320 or XL430-W250) consumes 0.4 ~ 0.6A of current)
- Support SynWrite function
- Support RC-100 library
- Serial communication using software serial library
- DYNAMIXEL Shield library(v0.1.0 or above) requires DYNAMIXEL2Arduino library

## Install Library
There are three ways to add libraries to the Arduino IDE.

- Using the Library Manager
- Importing a .zip Library
- Manual installation

Each way is described in detail in the [Arduino Official Guide], so please refer to it if necessary.  
Below is an example of using the Library Manager.

![](/img/parts/interface/dynamixel_shield/library_manager_01.webp)

Search for `dynamixel shield` from the Library Manager and install the latest version.

![](/img/parts/interface/dynamixel_shield/library_manager_03.webp)

Click `Install all` to install the dependent library `Dynamixel2Arduino`.

![](/img/parts/interface/dynamixel_shield/library_manager_04.webp)

If you have successfully installed both libraries, you'll be able to find several examples of DynamixelShield in the examples.

![](/img/parts/interface/dynamixel_shield/examples.webp)


:::warning

## Library API
### DYNAMIXELShield(v0.1.0 or above)
**WARNING** : In order to use DYNAMIXEL Shield library(v0.1.0 or above), [DYNAMIXEL2Arduino library](#install-library) must be installed.

:::


#### Dynamixel2Arduino Class
- [begin()]
- [getPortBaud()]
- [ping()]
- [scan()]
- [getModelNumber()]
- [setID()]
- [setProtocol()]
- [setBaudrate()]
- [torqueOn()]
- [torqueOff()]
- [ledOn()]
- [ledOff()]
- [setOperatingMode()]
- [setGoalPosition()]
- [getPresentPosition()]
- [setGoalVelocity()]
- [getPresentVelocity()]
- [setGoalPWM()]
- [getPresentPWM()]
- [setGoalCurrent()]
- [getPresentCurrent()]
- [readControlTableItem()]
- [writeControlTableItem()]

#### Master Class
Dynamixel2Arduino class inherits below public functions from the Master class.

- [syncRead()]
- [syncWrite()]
- [bulkRead()]
- [bulkWrite()]
- [getLastLibErrCode()]

#### RobotisRemoteController Class
- [begin()][rc100_begin]
- [availableData()]
- [readData()]
- [availableEvent()]
- [readEvent()]
- [flushRx()]
- [available()]
- [read()]
- [peek()]
- [flush()]
- [write()]


:::warning

### DynamixelShield(v0.0.5)
**WARNING** : This version of DYNAMIXEL Shield API is deprecated and not maintained anymore.

:::


```c
bool begin(uint32_t baud_rate = 57600, uint8_t protocol_version = DXL_PACKET_VER_2_0);

bool scan(void);
bool ping(uint8_t id = DXL_GLOBAL_ID);
bool addMotor(uint8_t id, uint8_t model);
bool setProtocolVersion(uint8_t version);
bool write(uint8_t id, uint16_t addr, uint8_t *p_data, uint16_t length, uint32_t timeout);
bool read(uint8_t id, uint16_t addr, uint8_t *p_data, uint16_t length, uint32_t timeout);

uint8_t getDxlCount(void);
uint8_t getDxlID(uint8_t index);
uint32_t getErr(void);
void clearErr(void);

bool reboot(uint8_t id);
bool reset(uint8_t id);

bool setID(uint8_t id, uint8_t new_id);
bool setBaud(uint8_t id, uint32_t new_baud);

bool ledOn(uint8_t id);
bool ledOff(uint8_t id);

bool torqueOn(uint8_t id);
bool torqueOff(uint8_t id);

bool setJointMode(uint8_t id);
bool setWheelMode(uint8_t id);

bool    setGoalPosition(uint8_t id, uint32_t position);
int32_t getGoalPosition(uint8_t id);
int32_t getCurPosition(uint8_t id);

bool    setGoalSpeed(uint8_t id, int32_t speed);
int32_t getGoalSpeed(uint8_t id);   
int32_t getCurSpeed(uint8_t id);  

bool    setGoalAngle(uint8_t id, int32_t angle);
int32_t getGoalAngle(uint8_t id);
int32_t getCurAngle(uint8_t id);

bool syncWriteBegin(void);
bool syncWriteEnd(void);
```

# Reference
## DYNAMIXEL Connectors
|           Item           |                             TTL                              |                            RS-485                            |                            XL-320 (TTL)                             |
|:------------------------:|:------------------------------------------------------------:|:------------------------------------------------------------:|:-------------------------------------------------------------------:|
|          Pinout          |                `1` GND<br />`2` VDD<br />`3` DATA                |         `1` GND<br />`2` VDD<br />`3` DATA+<br />`4` DATA-         |                   `1` GND<br />`2` VDD<br />`3` DATA                    |
|         Diagram          |        ![](/img/dxl/jst_b3beha_diagram.webp)        |        ![](/img/dxl/jst_b4beha_diagram.webp)        |         ![](/img/dxl/molex_532530370_diagram.webp)         |
|         Housing          |   ![](/img/dxl/JST_EHR-3.webp)<br />[JST EHR-03]    |   ![](/img/dxl/JST_EHR-4.webp)<br />[JST EHR-04]    | ![](/img/dxl/molex_510650300.webp)<br />[MOLEX 51065-0300] |
|        PCB Header        | ![](/img/dxl/JST_B3B_EH-A.webp)<br />[JST B3B-EH-A] | ![](/img/dxl/JST_B4B-EH-A.webp)<br />[JST B4B-EH-A] | ![](/img/dxl/molex_532530370.webp)<br />[MOLEX 53253-0370] |
|      Crimp Terminal      |                     [JST SEH-001T-P0.6]                      |                     [JST SEH-001T-P0.6]                      |                         [MOLEX 50212-8000]                          |
| Wire Gauge for DYNAMIXEL |                            21 AWG                            |                            21 AWG                            |                               24 AWG                                |

## Communication Circuit
To control the DYNAMIXEL actuators, the main controller needs to convert its UART signals to the half duplex type. The recommended circuit diagram for this is shown below.


### TTL Communication
![](/img/dxl/ttl_circuit.webp)

> TTL Circuit


:::note

**NOTE**: Above circuit is designed for 5V or 5V tolerant MCU. Otherwise, use a Level Shifter to match the voltage of MCU.

:::


![](/img/dxl/x/x_series_ttl_pin.webp)


### RS-485 Communication
![](/img/dxl/x/x_series_485_circuit.webp)  
> RS-485 Circuit 


:::note

**NOTE**: Above circuit is designed for 5V or 5V tolerant MCU. Otherwise, use a Level Shifter to match the voltage of MCU.

:::


![](/img/dxl/x/x_series_485_pin.webp)


The power of DYNAMIXEL is supplied via Pin1(-), Pin2(+).  
(The above circuit is built into DYNAMIXEL's controller only)  
In the above circuit diagram, the direction of data signal of TxD and RxD in the TTL Level is determined according to the level of DIRECTION 485 as follows:  
In case of DIRECTION485 Level = High: The signal of TxD is output to D+ and D-  
In case of DIRECTION485 Level = Low: The signal of D+ and D- is output to RxD  


# Download
- [DYNAMIXEL Shield Schematic(PDF)](https://www.robotis.com/service/download.php?no=2206)
  ![](/img/parts/interface/dynamixel_shield/dynamixel_shield_schematic01.webp)
  ![](/img/parts/interface/dynamixel_shield/dynamixel_shield_schematic02.webp)


[AX-12W]: /docs/dxl/model_reference/ax_series/ax-12w
[AX-12+/12A]: /docs/dxl/model_reference/ax_series/ax-12a
[AX-18F/18A]: /docs/dxl/model_reference/ax_series/ax-18a
[DX-113]: /docs/dxl/model_reference/dx_series/dx-113
[DX-116]: /docs/dxl/model_reference/dx_series/dx-116
[DX-117]: /docs/dxl/model_reference/dx_series/dx-117
[EX-106+]: /docs/dxl/model_reference/ex_series/ex-106+
[RX-10]: /docs/dxl/model_reference/rx_series/rx-10
[RX-24F]: /docs/dxl/model_reference/rx_series/rx-24f
[RX-28]: /docs/dxl/model_reference/rx_series/rx-28
[RX-64]: /docs/dxl/model_reference/rx_series/rx-64
[MX-12W]: /docs/dxl/model_reference/mx_series/mx-12w
[MX-28]: /docs/dxl/model_reference/mx_series/mx-28
[MX-28(2.0)]: /docs/dxl/model_reference/mx_series/mx-28-2
[MX-64]: /docs/dxl/model_reference/mx_series/mx-64
[MX-64(2.0)]: /docs/dxl/model_reference/mx_series/mx-64-2
[MX-106]: /docs/dxl/model_reference/mx_series/mx-106
[MX-106(2.0)]: /docs/dxl/model_reference/mx_series/mx-106-2
[XL320]: /docs/dxl/model_reference/x_series/xl_series/xl320
[XL430-W250]: /docs/dxl/model_reference/x_series/xl_series/xl430-w250
[XL330-M077]: /docs/dxl/model_reference/x_series/xl_series/xl330-m077
[XL330-M288]: /docs/dxl/model_reference/x_series/xl_series/xl330-m288
[2XL430-W250]: /docs/dxl/model_reference/x_series/xl_series/2xl430-w250
[XC430-W150]: /docs/dxl/model_reference/x_series/xc_series/xc430-w150
[XC430-W240]: /docs/dxl/model_reference/x_series/xc_series/xc430-w240
[XC430-T150BB]: /docs/dxl/model_reference/x_series/xc_series/xc430-t150bb
[XC430-T240BB]: /docs/dxl/model_reference/x_series/xc_series/xc430-t240bb
[2XC430-W250]: /docs/dxl/model_reference/x_series/xc_series/2xc430-w250
[XC330-T288]: /docs/dxl/model_reference/x_series/xc_series/xc330-t288
[XC330-T181]: /docs/dxl/model_reference/x_series/xc_series/xc330-t181
[XC330-M288]: /docs/dxl/model_reference/x_series/xc_series/xc330-m288
[XC330-M181]: /docs/dxl/model_reference/x_series/xc_series/xc330-m181
[XM430-W210]: /docs/dxl/model_reference/x_series/xm_series/xm430-w210
[XM430-W350]: /docs/dxl/model_reference/x_series/xm_series/xm430-w350
[XH430-W210]: /docs/dxl/model_reference/x_series/xh_series/xh430-w210
[XM540-W150]: /docs/dxl/model_reference/x_series/xm_series/xm540-w150
[XM540-W270]: /docs/dxl/model_reference/x_series/xm_series/xm540-w270
[XH430-W350]: /docs/dxl/model_reference/x_series/xh_series/xh430-w350
[XH430-V210]: /docs/dxl/model_reference/x_series/xh_series/xh430-v210
[XH430-V350]: /docs/dxl/model_reference/x_series/xh_series/xh430-v350
[XH540-W150]: /docs/dxl/model_reference/x_series/xh_series/xh540-w150
[XH540-W270]: /docs/dxl/model_reference/x_series/xh_series/xh540-w270
[XH540-V150]: /docs/dxl/model_reference/x_series/xh_series/xh540-v150
[XH540-V270]: /docs/dxl/model_reference/x_series/xh_series/xh540-v270
[XD540-T270-R]: /docs/dxl/model_reference/x_series/xd_series/xd540-t270
[XD540-T150-R]: /docs/dxl/model_reference/x_series/xd_series/xd540-t150
[XD430-T350-R]: /docs/dxl/model_reference/x_series/xd_series/xd430-t350
[XD430-T210-R]: /docs/dxl/model_reference/x_series/xd_series/xd430-t210
[XW430-T333-R]: /docs/dxl/model_reference/x_series/xw_series/xw430-t333
[XW430-T200-R]: /docs/dxl/model_reference/x_series/xw_series/xw430-t200
[XW540-T260-R]: /docs/dxl/model_reference/x_series/xw_series/xw540-t260
[XW540-T140-R]: /docs/dxl/model_reference/x_series/xw_series/xw540-t140
[H54-200-S500-R]: /docs/dxl/model_reference/pro_series/pro_h_series/h54-200-s500-r
[H54-100-S500-R]: /docs/dxl/model_reference/pro_series/pro_h_series/h54-100-s500-r
[H42-20-S300-R]: /docs/dxl/model_reference/pro_series/pro_h_series/h42-20-s300-r
[M54-60-S250-R]: /docs/dxl/model_reference/pro_series/pro_m_series/m54-60-s250-r
[M54-40-S250-R]: /docs/dxl/model_reference/pro_series/pro_m_series/m54-40-s250-r
[M42-10-S260-R]: /docs/dxl/model_reference/pro_series/pro_m_series/m42-10-s260-r
[H54-200-S500-R(A)]: /docs/dxl/model_reference/pro_series/pro_h_series/h54-200-s500-ra
[H54-100-S500-R(A)]: /docs/dxl/model_reference/pro_series/pro_h_series/h54-100-s500-ra
[H42-20-S300-R(A)]: /docs/dxl/model_reference/pro_series/pro_h_series/h42-20-s300-ra
[M54-60-S250-R(A)]: /docs/dxl/model_reference/pro_series/pro_m_series/m54-60-s250-ra
[M54-40-S250-R(A)]: /docs/dxl/model_reference/pro_series/pro_m_series/m54-40-s250-ra
[M42-10-S260-R(A)]: /docs/dxl/model_reference/pro_series/pro_m_series/m42-10-s260-ra
[L54-50-S500-R]: /docs/dxl/model_reference/pro_series/pro_l_series/l54-50-s500-r
[L54-50-S290-R]: /docs/dxl/model_reference/pro_series/pro_l_series/l54-50-s290-r
[L54-30-S500-R]: /docs/dxl/model_reference/pro_series/pro_l_series/l54-30-s500-r
[L54-30-S400-R]: /docs/dxl/model_reference/pro_series/pro_l_series/l54-30-s400-r
[L42-10-S300-R]: /docs/dxl/model_reference/pro_series/pro_l_series/l42-10-s300-r
[PH42-020-S300-R]: /docs/dxl/model_reference/p_series/ph42-020-s300-r
[PH54-100-S500-R]: /docs/dxl/model_reference/p_series/ph54-100-s500-r
[PH54-200-S500-R]: /docs/dxl/model_reference/p_series/ph54-200-s500-r
[PM54-060-S250-R]: /docs/dxl/model_reference/p_series/pm54-060-s250-r
[PM54-040-S250-R]: /docs/dxl/model_reference/p_series/pm54-040-s250-r
[PM42-010-S260-R]: /docs/dxl/model_reference/p_series/pm42-010-s260-r

[Arduino Official Guide]: https://www.arduino.cc/en/Guide/Libraries
[DYNAMIXEL2Arduino GitHub repository]: https://github.com/ROBOTIS-GIT/Dynamixel2Arduino
[DYNAMIXEL Shield GitHub repository]: https://github.com/ROBOTIS-GIT/DynamixelShield
[begin()]: /docs/software/arduino_ide/arduino_api/begin
[getPortBaud()]: /docs/software/arduino_ide/arduino_api/getPortBaud
[ping()]: /docs/software/arduino_ide/arduino_api/ping
[scan()]: /docs/software/arduino_ide/arduino_api/scan
[getModelNumber()]: /docs/software/arduino_ide/arduino_api/getModelNumber
[setID()]: /docs/software/arduino_ide/arduino_api/setID
[setProtocol()]: /docs/software/arduino_ide/arduino_api/setProtocol
[setBaudrate()]: /docs/software/arduino_ide/arduino_api/setBaudrate
[torqueOn()]: /docs/software/arduino_ide/arduino_api/torqueOn
[torqueOff()]: /docs/software/arduino_ide/arduino_api/torqueOff
[ledOn()]: /docs/software/arduino_ide/arduino_api/ledOn
[ledOff()]: /docs/software/arduino_ide/arduino_api/ledOff
[setOperatingMode()]: /docs/software/arduino_ide/arduino_api/setOperatingMode
[setGoalPosition()]: /docs/software/arduino_ide/arduino_api/setGoalPosition
[getPresentPosition()]: /docs/software/arduino_ide/arduino_api/getPresentPosition
[setGoalVelocity()]: /docs/software/arduino_ide/arduino_api/setGoalVelocity
[getPresentVelocity()]: /docs/software/arduino_ide/arduino_api/getPresentVelocity
[setGoalPWM()]: /docs/software/arduino_ide/arduino_api/setGoalPWM
[getPresentPWM()]: /docs/software/arduino_ide/arduino_api/getPresentPWM
[setGoalCurrent()]: /docs/software/arduino_ide/arduino_api/setGoalCurrent
[getPresentCurrent()]: /docs/software/arduino_ide/arduino_api/getPresentCurrent
[readControlTableItem()]: /docs/software/arduino_ide/arduino_api/readControlTableItem
[writeControlTableItem()]: /docs/software/arduino_ide/arduino_api/writeControlTableItem
[syncRead()]: /docs/software/arduino_ide/arduino_api/syncRead
[syncWrite()]: /docs/software/arduino_ide/arduino_api/syncWrite
[bulkRead()]: /docs/software/arduino_ide/arduino_api/bulkRead
[bulkWrite()]: /docs/software/arduino_ide/arduino_api/bulkWrite
[getLastLibErrCode()]: /docs/software/arduino_ide/arduino_api/getLastLibErrCode
[rc100_begin]: /docs/software/arduino_ide/arduino_api/rc100_begin
[availableData()]: /docs/software/arduino_ide/arduino_api/availableData
[readData()]: /docs/software/arduino_ide/arduino_api/readData
[availableEvent()]: /docs/software/arduino_ide/arduino_api/availableEvent
[readEvent()]: /docs/software/arduino_ide/arduino_api/readEvent
[flushRx()]: /docs/software/arduino_ide/arduino_api/flushRx
[available()]: /docs/software/arduino_ide/arduino_api/available
[read()]: /docs/software/arduino_ide/arduino_api/rc100_read
[peek()]: /docs/software/arduino_ide/arduino_api/peek
[flush()]: /docs/software/arduino_ide/arduino_api/flush
[write()]: /docs/software/arduino_ide/arduino_api/rc100_write
[JST EHR-03]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[JST EHR-04]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[JST B3B-EH-A]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[JST B4B-EH-A]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[JST SEH-001T-P0.6]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[MOLEX 51065-0300]: https://www.molex.com/molex/products/datasheet.jsp?part=active/0510650300_CRIMP_HOUSINGS.xml&channel=Product
[MOLEX 53253-0370]: https://www.molex.com/molex/products/datasheet.jsp?part=active/0532530370_PCB_HEADERS.xml
[MOLEX 50212-8000]: https://www.molex.com/molex/products/datasheet.jsp?part=active/0502128000_CRIMP_TERMINALS.xml
