...
To make the Dynamixel into previous condition, `dynamixel::PacketHandler::write1ByteTxRx` function sends an instruction to the #`DXL_ID` Dynamixel through the port which the `portHandler` handles, writing 1 bytes of `NEW_BAUDNUM` value to `ADDR_PRO_BAUDRATE` address. Then, it receives the `dxl_error`. The function returns 0 if no communication error has been occurred.
