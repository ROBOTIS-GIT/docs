...
| GREEN                                 | Green color constant for controller's RGB LED                | ``` led.on(const.GREEN) ```                       |
...
| ```write(position) -> None ```         | Set joint position of object of joint mode&lt;br&gt;(```mode```: const.OLLO_JOINT_POSITION)          | 0 ~ 1023    | None       | ``` obj.write(512) ```            |
...
| [```is_moving() -> bool ```](/docs/dxl/model_reference/x_series/xl_series/xl430-w250#moving122)       | Return moving status                 | None    | True : Dynamixel is moving&lt;br&gt;False : Dynamixel is not moving  | ``` obj.is_moving() ```            |
