...
- 예제 다운로드 하기 전, 로보플러스 매니저를 사용하여, CM-550의 펌웨어를 업데이트 하세요. [펌웨어 업데이트](/docs/software/rplus_manager_2_0/#firmware-update) 따라하기.
...
### 블루투스 연결하기
엔지니어 키트를 조종하기위해서, CM-550과 엔지니어 키트앱의 무선연결이 필요합니다. 엔지니어 키트1의 [블루투스 연결하기](/docs/edu/kr/engineer/kit1/#블루투스-연결하기)를 따라하세요.
...
- 스트리밍 사용방법은 [앱에서 영상 스트리밍 기능 설정하기](/docs/edu/kr/engineer/kit2_reference/#앱에서-영상-스트리밍-기능-설정하기) 참조하세요.
:::
...
|                          아이콘                          | 모드 설명                                                                                                                                          |
|:--------------------------------------------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------|
|  ![](/img/edu/engineer/icon_remote.webp)   | **REMOTE : 조종 모드**&lt;br /&gt;MAX-E2 로봇을 조종하는 모드이며 FIGHT, SOCCER 등의 모드 변경을 통해 MAX-E2를 다양한 모션으로 조종할 수 있습니다.         |
| ![](/img/edu/engineer/kit2_quickstart/icon_streaming.webp) | **STREAMING : 스트리밍 모드**&lt;br /&gt;Raspberry Pi 카메라를 활용해 전송되는 영상을 스마트 기기의 화면으로 보면서 버튼을 눌러 로봇을 조종할 수 있습니다. |
|   ![](/img/edu/engineer/icon_face.webp)    | **FACE : 얼굴 감지 모드**&lt;br /&gt;라즈베리파이 카메라를 활용해 얼굴을 감지하는 모드로 얼굴을 감지하면 인사 등의 지정된 동작을 수행합니다.               |

#### MAX-E2 옵션메뉴

|                        아이콘                         | 옵션 설명                                                                              |
|:-----------------------------------------------------:|:---------------------------------------------------------------------------------------|
| ![](/img/edu/engineer/icon_motor.webp)  | **MOTOR : 모터 점검**&lt;br /&gt;로봇의 다이나믹셀 상태를 각각 테스트해 볼 수 있는 메뉴입니다. |
| ![](/img/edu/engineer/icon_offset.webp) | **OFFSET : 모션 오프셋**&lt;br /&gt;모션 동작시 각 관절의 기본 위치를 설정하는 기능입니다.     |
...
**참고** : 옵션 메뉴는 모든 예제에 포함되어 있으며 세부 설정 방법은 [로봇 설정하기](/docs/edu/kr/engineer/kit2_reference/#로봇-설정하기)를 참고하세요.
...
`모드 버튼` :
  - `NORMAL MODE`를 선택하면 Scorpi 로봇이 일어나 동작하고, 절대거리센서 [DMS-80](/docs/edu/kr/engineer/kit2_introduction/#절대-거리-센서)에 물체가 감지되면 꼬리를 움직여 공격합니다.
  - `GUARD MODE`를 선택하면 Scorpi 로봇이 제자리에서 경계 자세를 취하며, 절대거리센서 [DMS-80](/docs/edu/kr/engineer/kit2_introduction/#절대-거리-센서)에 물체가 감지되면 꼬리를 움직여 공격합니다.

#### Scorpi 메뉴 기능 소개

|                         아이콘                         | 메뉴 설명                                                                                                                                                             |
|:------------------------------------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ![](/img/edu/engineer/icon_remote.webp)  | **REMOTE : 조종 모드** &lt;br /&gt;Scorpi 로봇을 조종하는 모드이며 로봇의 이동 및 방향 전환, 속도 변경과 로봇의 꼬리 및 집게를 사용하는 다양한 모션들을 동작시킬 수 있습니다. |
| ![](/img/edu/engineer/kit2_quickstart/icon_gesture.webp) | **GESTURE : 제스처 모드** &lt;br /&gt;스마트기기의 기울기, 흔들림 센서 등을 사용해 기기를 기울이거나 특정 동작을 통해 Scorpi 로봇의 이동 및 공격 등의 명령을 수행합니다.      |
...
[PC에서 파이썬 예제 다운로드 하기]: /docs/edu/engineer/python_download_pc
