...
**Note**: MX(2.0) is a special firmware for the DYNAMIXEL MX series providing support for DYNAMIXEL Protocol 2.0. MX(2.0) firmware can be uploaded to your servo using [Firmware Recovery](/docs/software/dynamixel_wizard_2_0/introduction) in DYNAMIXEL Wizard 2.0.
...
Each connected DYNAMIXEL must have a unique ID number. If DYNAMIXELs with the same ID are connected, packet collision and network problems will occur. To change a DYNAMIXEL's ID, reference the [DYNAMIXEL Control Table](/docs/software/dynamixel_wizard_2_0/basic_features#dynamixel-control-table) section on the DYNAMIXEL Wizard 2.0 eManual page.
...
- RAM data can be restored by configuring the **Startup Configuration(60)** setting. Refer to [Restoring RAM Area](/docs/software/dynamixel_wizard_2_0/advanced_features#restoring-ram-area) for more information.
