...
Be aware that the Profile Velocity(112) is to be only applied to **Position Control Mode**{props.currentBased === false ? <> or <strong>Extended Position Control Mode</strong></> : <>, <strong>Extended Position Control Mode</strong> or <strong>Current-based Position Control Mode</strong></>} on the [Operating Mode(11)].
