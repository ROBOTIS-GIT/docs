...
**NOTE** : If the Instruction Packet ID is set to the Broadcast ID(0xFE), Status Packets will not be returned for READ or WRITE Instructions regardless of the set value of Status Return Level ({props.srl ?? 68}). For more details, please refer to the `Status Packet` section for [DYNAMIXEL Protocol 2.0]
