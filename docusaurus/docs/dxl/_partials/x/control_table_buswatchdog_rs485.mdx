{/*
control_table_buswatchdog.mdx for the RS-485-only models (XD430, XD540, XW430-T333, XW540-T140/T260).
They have no TTL bus, so the opening sentence names RS-485 only.
*/}

The Bus Watchdog(98) is a safety device (Fail-safe) to stops the DYNAMIXEL if the communication between the controller and DYNAMIXEL communication (RS-485) is disconnected due to an unspecified error.
