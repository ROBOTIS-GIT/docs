{/*
control_table_buswatchdog.mdx for the XL430 group. These models have no Goal Current(102),
so it is not in the read-only Goal Value list.
*/}
...
If the Bus Watchdog Error screen appears, the Goal Value ([Goal PWM(100)], [Goal Velocity(104)], [Goal Position(116)]) will be changed to read-only-access.
