...
The Bus Watchdog(98) is a safety device (Fail-safe) to stops the DYNAMIXEL if the communication between the controller and DYNAMIXEL communication ({props.bus ?? 'RS-485, TTL'}) is disconnected due to an unspecified error.
...
If the Bus Watchdog Error screen appears, the Goal Value ([Goal PWM(100)], {props.current !== false && <><a href="#goal-current102">Goal Current(102)</a>, </>}[Goal Velocity(104)], [Goal Position(116)]) will be changed to read-only-access.
