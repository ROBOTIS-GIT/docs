{/* props
   shutdown, torqueEnable, hwError: 레지스터 주소. 기본 48 / 562 / 892 (PRO). PRO(A)와 RH-P12-RN은 63 / 512 / 518.
*/}
...
For instance, when ‘0x05’ (binary : 00000101) is defined in Shutdown({props.shutdown ?? 48}), DYNAMIXEL can detect both Input Voltage Error(binary : 00000001) and Overheating Error(binary : 00000100).
If those errors are detected,  [Torque Enable({props.torqueEnable ?? 562})](#torque-enable) is cleared to ‘0’ and the motor's output becomes 0 [%].


REBOOT is the only method to reset [Torque Enable({props.torqueEnable ?? 562})](#torque-enable) to ‘1’(Torque ON) after the shutdown.

 Check [Alert Bit(0x80)] in an error field of Status Packet or a present status via [Hardware Error Status({props.hwError ?? 892})](#hardware-error-status).  The followings are detectable situations.
