{/* props
   rh: RH-P12-RN(A) 페이지용 경고 문구로 바꾼다.
*/}
# Control Table
...
**WARNING** : {props.rh ? 'RH-P12-RN(A) uses different Control Table from RH-P12-RN. Please pay attention when upgrading RH-P12-RN to RH-P12-RN(A).' : 'PRO(A) series use different Control Table from DYNAMIXEL PRO series. Please pay attention when upgrading DYNAMIXEL PRO to PRO(A).'}
