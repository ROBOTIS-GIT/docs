In current control mode, the Goal Current is used to specify the desired output current. In velocity control mode and position control mode, it functions as a limit on the current controller's output current. Goal Current cannot exceed the configured [Current Limit(66)].
