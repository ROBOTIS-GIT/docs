...
9. Select `/detect/image_traffic_sign/compressed` topic from the drop down list. A screen will display the result of traffic sign detection.
    ...
    ![](/img/systems/turtlebot3/autonomous_driving/noetic_detect_intersection.webp)

    > Detecting the Intersection sign when `mission:=intersection`

    ![](/img/systems/turtlebot3/autonomous_driving/noetic_detect_left.webp)

    > Detecting the Left sign when `mission:=intersection`

    ![](/img/systems/turtlebot3/autonomous_driving/noetic_detect_right.webp)

    > Detecting the Right sign when `mission:=intersection`

    ![](/img/systems/turtlebot3/autonomous_driving/noetic_detect_construction.webp)

    > Detecting the Construction sign when `mission:=construction`

    ![](/img/systems/turtlebot3/autonomous_driving/noetic_detect_parking.webp)

    > Detecting the Parking sign when `mission:=parking`

    ![](/img/systems/turtlebot3/autonomous_driving/noetic_detect_level_crossing.webp)

    > Detecting the Level Crossing sign when `mission:=level_crossing`

    ![](/img/systems/turtlebot3/autonomous_driving/noetic_detect_tunnel.webp)
