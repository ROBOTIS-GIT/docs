...
**NOTE**: OpenMANIPULATOR-X controller is compatible with [Protocol 2.0](/docs/dxl/protocol/protocol2) which supports `MX 2.0`, `X` and `Pro` series. [Protocol 1.0](/docs/dxl/protocol/protocol1) does not support SyncRead instructions to access to multiple DYNAMIXEL's's simultaneously.
