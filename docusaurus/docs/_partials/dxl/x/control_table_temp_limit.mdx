...
If the operating DYNAMIXEL's internal temperature is over the Temperature Limit(11), the Status Packet transmits Overheating Error Bit (Bit2) via the ERROR field. Providing that Overheating flag is set in the [Alarm LED(17)/Shutdown(18)](#shutdown), the Alarm LED will start blinking and the motor's output will be 0 [%].
