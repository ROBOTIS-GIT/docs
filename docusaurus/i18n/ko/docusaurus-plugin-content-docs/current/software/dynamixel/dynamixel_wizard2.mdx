...
![](/img/software/dynamixel/wizard2/new_wizard2_main_002_kr.png)
...
![](/img/software/dynamixel/wizard2/new_wizard2_graph_001_kr.png)
...
![](/img/software/dynamixel/wizard2/new_wizard2_packet_001_kr.png)
...
1. 메인 메뉴에서 `도구` > `설정` 를 선택하면 설정창이 나타납니다.
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_011_kr.png)
    
    

2. 설정창의 좌측 목록에서 `검색` 항목을 선택하면 검색 설정이 나타납니다.

    ![](/img/software/dynamixel/wizard2/new_wizard2_options_001_kr.png)
...
1. 메인 메뉴에서 `장치` > `검색` 을 선택하면 다이나믹셀 검색을 시작합니다.
    ...
    **참고** : 만약, 해당 통신 포트에 연결을 실패하면 사용중인 프로그램을 찾아 사용을 해제해야 합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_012_kr.png)

2. 검색되는 다이나믹셀을 왼쪽의 장치목록에서 확인할 수 있습니다.  
    
    ![](/img/software/dynamixel/wizard2/new_wizard2_001_kr.png)
...
2. 장치목록은 포트, 통신속도, 장치 순으로 분류합니다. 테스트할 그룹이나 장치를 선택합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_002_kr.png)
...
3. 부가 메뉴에서 `팩토리 리셋` 버튼을 클릭하면 제품을 공장 초기화 상태로 되돌릴 수 있습니다(이때, ID와 Baudrate 값은 변경하지 않습니다).  
  ...
  부가 메뉴에서 `LED` 버튼을 클릭하면 해당 다이나믹셀의 LED를 켜거나 끌 수 있습니다.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_003_kr.png)
...
4. 부가 메뉴의 `토크` 버튼을 클릭하여 다이나믹셀의 토크를 켭니다.
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_torque_on_kr.png)  
...
6. 우측 하단의 위치 또는 속도 값 컨트롤을 사용하여 모터를 움직여봅니다.
    ...
    **경고** : 값을 조정하는 과정에서 모터가 갑작스럽게 움직일 수 있으니 주의하세요.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_004_position_mode_joypad_kr.png)  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_004_velocity_mode_joypad_kr.png)  
...
2. 설정창의 좌측 목록에서 `그래프` 항목을 선택하면 그래프 설정이 나타납니다.
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_002_kr.png)

    

3. 아래 그림에 표시된 `>` 버튼을 눌러 항목을 추가 합니다. (`<` 버튼으로 항목을 제거할 수 있습니다.)

    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_003_kr.png)

    

4. 추가된 항목을 더블 클릭하면 항목 추가 상자가 나타납니다.

    ![](/img/software/dynamixel/wizard2/wizard2_graph_004_kr.png)

5. 항목 설정 상자에서 색깔, Y축을 설정할 수 있습니다.

    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_004_1_kr.png)
    ...
    **참고**: 최신버전에서는 `비율`(Scale)과 `오프셋`(Offset) 선택 옵션은 삭제되고, [그래프 창](#그래프-창)에서 실시간으로 조정합니다. (아래 y축 설명 참고)
    ...
    - Y축: 선택된 항목의 Y축을 그래프에 표시합니다. 총 10개의 Y축을 그래프에 나타낼 수 있습니다.
      ...
      ![](/img/software/dynamixel/wizard2/wizard2_graph_004_y_axis_02_new_kr.png)
...
1. 메인 메뉴에서 `보기` > `그래프` 를 선택하면 그래프창이 나타납니다.
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_013_kr.png)
...
2. 그래프창의 `시작` 버튼을 클릭하면 그래프가 동작을 시작합니다.
    ...
    **참고** : 선택된 장치가 없으면 해당 메뉴가 비활성화된 상태입니다. 검색 후 다이나믹셀이 연결된 상태에서 진행해 주세요.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_start_001_kr.png)

    

3. 컨트롤 테이블에서 `Goal Position` 항목의 값을 변경하면 실시간으로 변경되는 `Present Position` 을 확인할 수 있습니다.

    ![](/img/software/dynamixel/wizard2/wizard2_graph_start_005_kr.png)

    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_start_006_kr.png)  
...
5. 다음의 메뉴를 통해 그래프를 제어할 수 있습니다.
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_start_002_kr.png)  
...
1. 메인 메뉴에서 `보기` > `패킷` 를 선택하면 패킷창이 나타납니다.
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_014_kr.png)
...
2. 통신포트와 통신속도를 설정하고 `열기` 버튼을 클릭합니다.  
  열기에 성공하면 해당 포트의 다이나믹셀 프로토콜 통신 모니터링을 시작합니다.
    ...
    **참고** : 만약, 해당 통신 포트에 연결을 실패하면 사용중인 프로그램을 찾아 사용을 해제해야합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_packet_002_kr.png)
...
3. 우측 메뉴에서 통신할 프로토콜 버전과 Instruction을 설정할 수 있습니다.  
  프로토콜 버전을 `2.0` 으로 Instruction을 `Write` 로 설정합니다.  
    ...
    **참고** : 각 다이나믹셀은 지원하는 프로토콜 버전이 다릅니다. **관련 다이나믹셀의 [프로토콜 버전](/docs/popup/faq_protocol_compatibility_table)을 반드시 확인하세요.**
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_packet_003_kr.png)
...
4. Instuction에 대한 상세설정을 할 수 있습니다.  
  다음과 같이 LED 켜기의 패킷을 설정합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_packet_004_kr.png)
...
5. 설정된 값에 따라 변화되는 패킷정보를 하단에 있는 `패킷상세정보` 에서 확인할 수 있습니다.  
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_packet_005_kr.png)

6. `전송` 버튼을 클릭하여 패킷을 전송합니다.  

    ![](/img/software/dynamixel/wizard2/new_wizard2_packet_006_kr.png)
    
    

7. 전송 후 패킷을 클릭하면 상세정보를 아래에서 확인할 수 있습니다.

    ![](/img/software/dynamixel/wizard2/new_wizard2_packet_007_kr.png)

## 펌웨어 업데이트
다이나믹셀의 펌웨어 업데이트가 필요할 경우, 장치이름 앞에 특수문자(*)와 함께 연하게 표시됩니다.  
해당 다이나믹셀에 마우스를 올리면, **“펌웨어 업데이트가 있습니다” ** 문장이 표시됩니다.  

![](/img/software/dynamixel/wizard2/new_wizard2_update_notification_kr.png)
...
1. 메인 메뉴에서 `도구` > `펌웨어 업데이트`를 선택합니다.
    ...
    **참고** : 선택된 장치가 없으면 해당 메뉴가 비활성화된 상태입니다. 검색 후 다이나믹셀이 연결된 상태에서 진행해 주세요.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_015_kr.png)

    

2. 간단한 도움말과 함께 펌웨어 업데이트를 시작합니다. 특히, 업데이트 도중 연결이 끊기거나 전원이 꺼지지 않도록 유의하십시오.

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_update_001_kr.png)

3. 다음을 누르면 펌웨어 업데이트를 시작합니다. 완료될때 까지 전원이 꺼지거나 케이블이 빠지지 않도록 유의합니다.

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_update_002_kr.png)

4. 다이나믹셀 펌웨어 설치 결과를 학인합니다.

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_update_004_kr.png)

## 펌웨어 전체 업데이트
1. `도구` > `펌웨어 전체 업데이트`을 선택합니다.

    ![](/img/software/dynamixel/wizard2/new_wizard2_016_kr.png)

    

2. 펌웨어 업데이트가 필요한 다이나믹셀을 선택후, `다음` 버튼을 클릭합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_update_all_001_kr.png)

3. 펌웨어 업데이트가 진행됩니다.

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_update_all_002_kr.png)

4. 펌웨어 업데이트 완료후, `닫기` 버튼을 클릭합니다.

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_update_all_003_kr.png)

## 펌웨어 복구
1. 메인 메뉴에서 `도구` > `펌웨어 복구`를 선택합니다.

    ![](/img/software/dynamixel/wizard2/new_wizard2_017_kr.png)
    
    

2. 간단한 도움말과 함께 다이나믹셀 펌웨어 복구 마법사를 시작합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_recovery_001_kr.png)

3. 다이나믹셀 펌웨어 복구 모드에서는 모델명을 자동으로 인식할 수 없기 때문에, 사용자가 정확한 모델을 선택해 주어야 합니다. 잘못된 모델을 선택하면 정상적으로 동작하지 않습니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_recovery_002_kr.png)

4. 복구할 펌웨어 버전을 선택하십시오.

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_recovery_003_kr.png)
...
5. 사용자가 다이나믹셀이 연결된 포트를 수동으로 설정해 주어야 합니다. 포트가 사용중이면 다이나믹셀을 인식할 수 없으니 다른 프로그램을 종료하고 진행하십시오.  
    ...
    **주의** : 다이나믹셀 펌웨어 복구시에는 반드시 한개의 다이나믹셀만 연결되어 있어야 합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_firmware_recovery_004_kr.png)

6. 다이나믹셀을 찾기 위해 복구할 다이나믹셀의 전원을 껐다가 켜십시오.  

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_recovery_005_kr.png)

7. 다이나믹셀 검색에 성공하면 펌웨어 복구를 시작합니다. 완료될 때까지 전원이 꺼지거나 케이블이 빠지지 않도록 주의하십시오.  

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_recovery_006_kr.png)

8. 다이나믹셀 펌웨어 복구 결과를 확인합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_firmware_recovery_007_kr.png)
...
**지원 다이나믹셀**
- [DYNAMIXEL-X](/docs/dxl/x) (펌웨어 버전 **45 이상**, X330 시리즈: 펌웨어 버전 **46 이상**) 
- [DYNAMIXEL-P](/docs/dxl/p) (펌웨어 버전 **12 이상**)
- XL-320은 지원되지 않습니다.
...
1. 도구 > `ID 진단`을 클릭하세요.
    ...
    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step1_01_kr.png)  

2. 다이나믹셀과 연결된 USB 포트 및 다이나믹셀의 보드레이트를 선택후, `검색`을 클릭하세요.  

    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step1_02_kr.png)  

3. 다이나믹셀 검색이 시작되면, 상태창에 해당 다이나믹셀의 ID가 나타납니다.

    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step2_01_kr.png)  
...
4. 검색이 끝나면, `다음` 버튼을 클릭하여, **ID 설정** 단계로 이동하세요.
    ...
    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step2_02_kr.png)  

    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step3_01_kr.png)  

5. ID 설정 단계에서, 중복된 ID를 가진 다이나믹셀은 빨간색으로 표시됩니다. ID를 새롭게 지정 후 `저장`을 클릭하세요.

    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step3_02_kr.png)  
...
6. 중복된 ID가 아닌 항목은 초록색으로 표시됩니다. ID 할당이 끝나면, `적용`을 클릭하여, 저장된 ID를 다이나믹셀에 할당하세요.
    ...
    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step4_01_kr.png)  

7. `끝` 을 클릭하여, `ID 진단`을 종료합니다.

    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_step4_02_kr.png)  

7. 다이나믹셀 검색 시, `ID 진단`에서 할당한 ID가 성공적으로 적용된것을 확인할수 있습니다.

    ![](/img/software/dynamixel/wizard2/id_inspection/id_inspection_done_kr.png)  
...
2. 백업을 사용하기에 앞서, 아래와 같이 다이나믹셀이 **Torque Off** 상태인지 확인하세요. 그렇지 않으면, 백업에 실패합니다.
    ...
    ![](/img/software/dynamixel/wizard2/backup/backup_torque_off_kr.png)
    
3. `도구` > `컨트롤 테이블` > `백업` 을 클릭하세요.

    ![](/img/software/dynamixel/wizard2/backup/backup_click_backup_kr.png)
...
1. `도구` > `컨트롤 테이블` > `EEPROM 복구`를 클릭하세요.
    ...
    ![](/img/software/dynamixel/wizard2/backup/backup_click_restore_kr.png)
...
![](/img/software/dynamixel/wizard2/backup/backup_save_ctd_kr.png)

EEPROM 영역의 특정 데이터 (ID, Baudrate, Protocol Type)의 복구 여부를 선택할 수 있습니다. 

![](/img/software/dynamixel/wizard2/backup/backup_load_01_kr.png)

현재값과 다른 항목만 변경됩니다.

![](/img/software/dynamixel/wizard2/backup/backup_load_02_kr.png)
...
1. 메인 메뉴의 `도움말` > `다이나믹셀 펌웨어 정보` 메뉴를 클릭합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_009_kr.png)

2. 선택된 제품의 펌웨어 버전, 배포일, 업데이트 노트를 확인할 수 있습니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_010_kr.png)
...
2. 메인 메뉴의 `도구` > `캘리브레이션` 메뉴를 클릭합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_cali_001_kr.png)

    
    
    **참고**: 캘리브레이션을 지원하지 않는 모델(예: [AX 시리즈](/docs/dxl#ax-시리즈))은, 메뉴에서 `캘리브레이션`이 활성화되지 않습니다. 
...
3. 간단한 도움말과 함께 다이나믹셀 캘리브레이션을 시작합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_cali_002_kr.png)

4. 다이나믹셀을 찾기 위해 다이나믹셀의 전원을 껐다가 켜십시오.  

    ![](/img/software/dynamixel/wizard2/wizard2_cali_005_kr.png)

5. 다이나믹셀 검색에 성공하면 캘리브레이션 펌웨어를 설치합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_cali_006_kr.png)

    ![](/img/software/dynamixel/wizard2/wizard2_cali_007_kr.png)

6. 첫번째 위치를 설정합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_cali_008_kr.png)

7. 두번째 위치를 설정합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_cali_009_kr.png)

8. 세번째 위치를 설정합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_cali_010_kr.png)

9. 네번째 위치를 설정합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_cali_011_kr.png)

10. 캘리브레이션이 완료되었습니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_cali_012_kr.png)
...
1. 메인 메뉴의 `도구` > `자가진단` 메뉴를 클릭합니다.  
    ...
    **참고**: 자가진단 기능을 지원하지 않는 모델(예: [RX 시리즈](/docs/dxl#rx-시리즈))은, 메뉴에서 `자가진단`이 활성화되지 않습니다. 
    

    ![](/img/software/dynamixel/wizard2/wizard2_self_001_kr.png)
...
2. 간단한 도움말과 함께 자가진단을 시작합니다.  
    ...
    **주의** : 다이나믹셀 자가진단시에는 공장 초기화 상태로 변경됩니다.  
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_self_002_kr.png)

3. 자가진단을 진행할 다이나믹셀의 모델명과 펌웨어 버전을 확인합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_self_003_kr.png)

4. 다이나믹셀의 하드웨어 에러 상태를 확인합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_self_004_kr.png)

5. 다이나믹셀의 동작 온도가 정상범위인지 확인합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_self_005_kr.png)

6. 다이나믹셀의 입력 전압이 정상범위인지 확인합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_self_006_kr.png)

7. 다이나믹셀의 LED가 제대로 깜박이는지 확인합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_self_007_kr.png)

8. 다이나믹셀의 혼이 센터 위치로 이동했는지 확인합니다.  

    ![](/img/software/dynamixel/wizard2/wizard2_self_008_kr.png)
...
10. 자가진단 결과를 확인합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_self_010_kr.png)
...
1. 윈도우의 `장치 관리자`를 실행합니다. 해당 포트를 선택하고 마우스 오른쪽 버튼을 클릭하여 나온 메뉴에서 `속성`을 선택합니다.
    ...
    ![](/img/software/dynamixel/wizard2/wizard2_graph_008.png)

2. 속성의 `포트 설정` 탭에서 `고급` 버튼을 클릭합니다.

    ![](/img/software/dynamixel/wizard2/wizard2_graph_009.png)

3. 고급 설정에서 `응답 시간`을 `1ms`로 설정합니다.

    ![](/img/software/dynamixel/wizard2/wizard2_graph_010.png)
...
1. 그래프 설정에서 `통신 주기` 을 `1ms` 로 설정하고 `선택항목만 읽어오기` 항목을 체크합니다.
    ...
    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_005_kr.png)

    

2. 다이나믹셀의 통신속도(Baud Rate)를 최대로 설정하고 `Return Delay Time` 을 `0` 으로 설정합니다.

    ![](/img/software/dynamixel/wizard2/new_wizard2_graph_007_kr.png)
...
- 패킷의 경우 모니터링된 모든 패킷을 파일로 저장합니다.  
    ...
    ![](/img/software/dynamixel/wizard2/logging/wizard2_logging_kr.png)

#### 그래프 로깅
그래프를 시작하면 “문서/ DYNAMIXEL Wizard2/graph” 폴더에 저장됩니다.  
전체 경로가 제목표시줄에 출력됩니다.  

![](/img/software/dynamixel/wizard2/logging/wizard2_graph_logging_kr.png)

#### 패킷 로깅
통신포트를 열면 “문서/ DYNAMIXEL Wizard2/packet” 폴더에 저장됩니다.   
전체 경로가 제목표시줄에 출력됩니다.

![](/img/software/dynamixel/wizard2/logging/wizard2_packet_logging_kr.png)


[기본 기능 > 그래프]: #그래프
[프로토콜 1.0]: /docs/dxl/protocol1
[프로토콜 2.0]: /docs/dxl/protocol2
