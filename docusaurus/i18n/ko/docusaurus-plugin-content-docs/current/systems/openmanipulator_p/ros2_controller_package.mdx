...
**NOTE**: OpenMANIPULATOR-P controller is compatible with [Protocol 2.0](/docs/dxl/protocol2). [Protocol 1.0](/docs/dxl/protocol1) doesn't support SyncRead instructions that access to multiple DYNAMIXEL's simultaneously. Protocol 2.0 supports `MX 2.0`, `X`, `Pro`, `P` series, but it does not support `AX`, `RX` and `EX`.
