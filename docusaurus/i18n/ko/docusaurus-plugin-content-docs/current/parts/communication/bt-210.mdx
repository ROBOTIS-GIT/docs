...
# 개요
![img](/img/parts/communication/bt-210_product.webp)
...
# BT-210 설정하기
## 설정
1. OpenCM 9.04에 [Arduino IDE](/docs/software/arduino_ide)의 "c_Serial_Serial2_Converter" 또는 "f_SerialUSB_Serial2_Converter" 예제를 다운로드 합니다.
  
    [OpenCM9.04를 아두이노 IDE에 포팅하기](/docs/parts/controller/opencm904#아두이노-ide)

    ![img](/img/parts/communication/bt-210_02_arduino.webp)

    ![img](/img/parts/communication/bt-210_02.webp)

2. OpenCM9.04와 BT-210을 연결한 후, Tera Term을 실행합니다.

    - `다운로드` [Tera Term](https://osdn.net/projects/ttssh2/releases/)

    ![img](/img/parts/communication/bt-210_03.webp)

3. OpenCM-9.04와 PC를 USB를 연결하고 드라이버 설치 후 해당 COM포트로 Tera Term 터미널 프로그램으로 연결합니다.

    ![img](/img/parts/communication/bt-210_04.webp)

4. 아래와 같이 표시되면 연결이 완료되었습니다. baud rate는 57600으로 설정해주세요. 

    ![img](/img/parts/communication/bt-210_05.webp)
...
1. 시스템 리셋
  ATZ를 입력 후 엔터를 누르면 아래와 같이 BT-210이 재시작 됩니다. 만약 설정을 변경하였다면 변경된 모드로 재시작합니다.
    ...
    ![img](/img/parts/communication/bt-210_09.webp)
...
2. 모드 변경
  BT-210의 디바이스 모드(Slave/Master)를 설정하기 위해서는 아래의 커맨드로 변경을 할 수 있습니다. (대소문자 관계 없음)
    ...
    ![img](/img/parts/communication/bt-210_10.webp)

    > Master 모드

    ![img](/img/parts/communication/bt-210_11.webp)
...
3. 주변장치 검색
  주변 장치 검색은 BT-210이 MASTER모드일 때만 동작합니다. 아래와 같이 AT+BTINQ를 입력 후 엔터를 누르면 현재 주변에 Device Class가 0x1F00(SPP)인 Device가 차례로 검색되어 나옵니다
    ...
    ![img](/img/parts/communication/bt-210_12.webp)
...
8. Device Under Test 모드(DUT)
  DUT모드는 블루투스 테스터기에서 디바이스의 성능을 측정할 때 사용되는 모드이고 일반적인 용도에서는 쓰이지 않습니다.
    ...
    ![img](/img/parts/communication/bt-210_18.webp)

    DUT모드에서 빠져나오기 위해서는 아래와 같이 한번 더 AT+BTDUT를 입력하시면 Disable되고 ATZ를 통해 리셋하시면 됩니다.

    ![img](/img/parts/communication/bt-210_19.webp)
...
# 핀 배열
![img](/img/parts/communication/bt-210_01.webp)
...
[CM-530]: https://emanual.robotis.com/docs/en/parts/connector/cm-530/
[CM-700]: /docs/parts/controller/cm-700
[CM-900]: /docs/parts/controller/cm-900
[OpenCM9.04]: /docs/parts/controller/opencm904
[R+ Manager 2.0]: /docs/software/dynamixel/dynamixel_wizard2#사용전-주의사항/
[R+ Manager 1.0]: https://emanual.robotis.com/docs/en/software/rplus1/manager#펌웨어-관리/
