...
# 개요
![img](/img/parts/communication/bt-410_product.webp)
...
1. OpenCM 9.04에 [Arduino IDE](/docs/software/arduino_ide)의 `f_SerialUSB_Serial2_Converter` 예제를 다운로드 합니다.
...
7. 터미널 설정을 마치고 BT-410을 OpenCM-9.04에서 떼었다가 붙이면 아래와 같이 SLAVE/MASTER 모드 중에 하나로 작동됩니다. MASTER 모드로 동작하는 경우 아래 그림과 같이 표시됩니다.
    ...
    ![img](/img/parts/communication/bt-410_02.gif)
...
8. 주변 장치 검색은 BT-410이 MASTER모드일 때만 동작합니다. 아래와 같이 `AT+BTINQ`를 입력 후 엔터를 누르면 현재 주변에 Device Class가 0x1F00(SPP)인 Device가 차례로 검색되어 나옵니다
    ...
    ![img](/img/parts/communication/bt-410_03.webp)
...
9. 장치 연결은 `AT+BTINQ` 커맨드로 주변 장치 검색을 하고 난 뒤 표시된 블루투스 어드레스(16진수 12자리)를 통해 페어링 연결 합니다.  
  ...
  ![img](/img/parts/communication/bt-410_04.webp)
...
1. 시스템 리셋  
  `ATZ`를 입력 후 엔터를 누르면 아래와 같이 BT-410이 재시작 됩니다. 만약 설정을 변경하였다면 변경된 모드로 재시작합니다.
    ...
    ![img](/img/parts/communication/bt-410_05.webp)
...
2. 주변장치 검색  
  주변 장치 검색은 BT-410이 MASTER모드일 때만 동작합니다. 아래와 같이 `AT+BTINQ`를 입력 후 엔터를 누르면 현재 주변에 Device Class가 0x1F00(SPP)인 Device가 차례로 검색되어 나옵니다.
    ...
    ![img](/img/parts/communication/bt-410_06.webp)
...
3. 장치 연결(페어링)  
  ...
  여기서 장치간의 Pin Code가 맞지 않으면 페어링이 안되기 때문에 반드시 체크를 해야 합니다.(기본적으로 `0000` 입니다.)
    ...
    ![img](/img/parts/communication/bt-410_07.webp)

4. Bluetooth 장치 이름 변경  
  `AT+BTNAME = 원하는 장치 이름`

    ![img](/img/parts/communication/bt-410_08.gif)

5. 페어링 재설정

    ![img](/img/parts/communication/bt-410_09.gif)
...
6. 자동 페어링 비활성화 및 활성화 (**REV2, 파란색 스티커 MASTER모듈만 가능**)  
    ...
    ![img](/img/parts/communication/bt-410_10.webp)

    몇 초 뒤에 성공메시지 출력됨.

    ![img](/img/parts/communication/bt-410_11.webp)

    - **활성화** : `AT+BTAUTOPAIR=1` 을 입력 후 ENTER 키 입력

    ![img](/img/parts/communication/bt-410_12.webp)

    몇 초 뒤에 성공메시지 출력됨.

    ![img](/img/parts/communication/bt-410_13.webp)
...
# 핀 배열
![img](/img/parts/communication/bt-410_01.webp)
