...
<img src="/img/systems/openmanipulator_x/real_omx_init_pose.webp" width="250" />
...
**TIP**:
- If you can't load DYNAMIXEL, please check firmware to use ROBOTIS software ([DYNAMIXEL Wizard 2.0](/docs/software/dynamixel_wizard_2_0/basic_features#firmware-update))  
...
**NOTE**: open_manipulator_x controller is compatible with [Protocol 2.0](/docs/dxl/protocol/protocol2) which supports `MX 2.0`, `X` and `Pro` series. [Protocol 1.0](/docs/dxl/protocol/protocol1) does not support SyncRead instructions to access to multiple DYNAMIXEL's's simultaneously.
