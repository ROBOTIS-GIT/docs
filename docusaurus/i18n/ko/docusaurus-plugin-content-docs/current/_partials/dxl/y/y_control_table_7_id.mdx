...
**참고** : Instruction packet의 ID가 Broadcast ID(0xFE)인 경우, [Status Return Level(15)]의 설정값과 무관하게 Read Instruction 또는 Write Instruction에 대한 Status Packet은 반환되지 않습니다. 더 자세한 설명은 [다이나믹셀 프로토콜 2.0](/docs/dxl/protocol2#status-packet)의 Status Packet 항목을 참조하세요.
