| 항목         | RS-485                                                     | 외부포트                                                            | Dual Joint                                                          |
| :----------: | :--------------------------------------------------------: | :-----------------------------------------------------------------: | :-----------------------------------------------------------------: |
| 핀 번호      | `1` GND<br />`2` VDD<br />`3` DATA+<br />`4` DATA-               | `1` GND<br />`2` VDD<br />`3` PORT 1<br />`4` PORT 2<br />`5` PORT 3        | `1` PWM1<br />`2` PWM2<br />`3` ENABLE                                  |
| 다이어그램   | ![](/img/common/jst_b4beha_diagram.webp)             | ![](/img/dxl/model_reference/x_series/molex_5304705_diagram.webp)                   | ![](/img/dxl/model_reference/common/molex_588988000_diagram.webp)                 |
| 하우징       | ![](/img/common/JST_EHR-4.webp)<br />[JST EHR-04]   | ![](/img/dxl/model_reference/x_series/molex_510210500.webp)<br />[MOLEX 51021-0500] | ![](/img/dxl/model_reference/common/molex_510210300.webp)<br />[MOLEX 51021-0300] |
| PCB Header     | ![](/img/common/JST_B4B-EH-A.webp)<br />[JST B4B-EH-A] | ![](/img/dxl/model_reference/x_series/molex_530470510.webp)<br />[MOLEX 53047-0510] | ![](/img/dxl/model_reference/common/molex_533980371.webp)<br />[MOLEX 53398-0371] |
| Crimp Terminal | [JST SEH-001T-P0.6]                                        | [MOLEX 50079-8100]                                                  | [MOLEX 50058-8000]                                                  |
| Wire Gauge for DYNAMIXEL   | 21 AWG                                                     | 26 AWG                                                              | 26 AWG                                                              |

:::danger

**주의** : 커넥터 제조사에 따라 PIN 순서가 다를 수 있습니다. 사용자의 안전을 보호하고 재산상의 손해를 막기 위해 반드시 PIN 순서를 확인하세요.

:::

[JST EHR-04]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[JST B4B-EH-A]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[JST SEH-001T-P0.6]: http://www.jst-mfg.com/product/pdf/eng/eEH.pdf
[MOLEX 51021-0500]: http://www.molex.com/molex/products/datasheet.jsp?part=active/0510210500_CRIMP_HOUSINGS.xml
[MOLEX 53047-0510]: http://www.molex.com/molex/products/datasheet.jsp?part=active/0530470510_PCB_HEADERS.xml
[MOLEX 50079-8100]: http://www.molex.com/molex/products/datasheet.jsp?part=active/0500798100_CRIMP_TERMINALS.xml
[MOLEX 53398-0371]: https://uk.farnell.com/molex/53398-0371/header-smt-vertical-1-25mm-3way/dp/1125353
[MOLEX 51021-0300]: https://www.korean.molex.com/molex/products/datasheet.jsp?part=active/0510210300_CRIMP_HOUSINGS.xml
[MOLEX 50058-8000]: https://www.korean.molex.com/molex/products/datasheet.jsp?part=active/0500588000_CRIMP_TERMINALS.xml
