Torque ON/OFF 상태를 제어합니다. 값을 `1`로 설정하면 Torque ON State가 되고 EEPROM 영역의 모든 Data 쓰기가 제한됩니다. Torque ON을 수행할 수 없는 경우 반환되는 Status Packet의 Result Fail bit가 `1`로 설정됩니다.
