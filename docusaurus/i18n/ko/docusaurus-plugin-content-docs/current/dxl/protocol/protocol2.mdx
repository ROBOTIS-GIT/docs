...
* [DYNAMIXEL Y](/docs/dxl/model_reference/y_series)
* [DYNAMIXEL P](/docs/dxl/model_reference/p_series)
* [DYNAMIXEL X](/docs/dxl/model_reference/x_series)
* [DYNAMIXEL MX](/docs/dxl/model_reference/mx_series)

:::note

**참고**: MX(2.0)은 DYNAMIXEL Protocol 2.0을 지원하기 위한 DYNAMIXEL MX 시리즈 전용 펌웨어입니다. DYNAMIXEL Wizard 2.0의 [Firmware Recovery](/docs/software/dynamixel_wizard_2_0/introduction)를 사용해 서보에 MX(2.0) 펌웨어를 업로드할 수 있습니다.
...
**팁**: 자세한 내용은 DYNAMIXEL Protocol [Compatibility Table]을 참고하세요.
...
그러므로 ID가 같은 DYNAMIXEL이 존재하지 않도록 ID설정을 해야 합니다. ID 설정을 위해서는, DYNAMIXEL 위자드 2.0에서 [컨트롤 테이블](/docs/software/dynamixel_wizard_2_0/basic_features#dynamixel-control-table)을 참고 하세요.
...
![](/img/dxl/protocol/halfduplex.webp)
...
![](/img/dxl/protocol/protocol2/protocol20_BytetoByteTime.webp)
...
![](/img/dxl/protocol/protocol2/protocol20_packet_example_02.webp)
...
**주의**: DYNAMIXEL Broadcast ID(254 (0xFE))는 [Ping], [Sync Read], [Bulk Read] 명령에 대해서만 [Status Packet](#status-packet)을 반환합니다. [Ping], [Sync Read], [Bulk Read]를 제외한 다른 [Instruction](#instruction)은 Broadcast ID로 전송해도 Status Packet을 반환하지 않습니다.
...
Packet이 통신 중에 파손되었는지를 점검하기 위한 필드 (16bit CRC)
  ...
  3. CRC 계산 방법 및 예제 : [CRC Calculation](/docs/dxl/protocol/crc)

## Status Packet

Status Packet은 장치(Device)가 [Instruction Packet]을 수신한 뒤 Main Controller로 전송하는 응답 Packet입니다. Error 필드가 추가되는 것을 제외하면 Instruction Packet과 구성이 동일합니다.
...
| Alert | Error Number  |
  ...
  - Alert : 장치에 하드웨어적인 문제가 발생한 경우 1로 설정됩니다. 이 경우 [Control Table]의 Hardware error status 값을 읽으면 문제의 원인을 확인할 수 있습니다.
...
|     0x02     | Instruction Error | 정의되지 않은 Instruction을 사용한 경우<br />Reg Write 없이 Action 을 사용한 경우                                                                                                                   |
...
|     0x07     | Access Error      | Read Only 혹은 정의되지 않은 Address에 값을 쓰려고 한 경우<br />Write Only 혹은 정의되지 않은 Address에 값을 읽으려고 한 경우<br />Torque Enable(ROM Lock) 상태에서 ROM 영역에 값을 쓰려고 한 경우 |
...
[Control Table]의 Status Return Level 값에 따라 응답 여부가 결정됩니다. 각 장치의 Status Return Level에서 설정 가능한 값을 참고하세요.

:::warning

**주의**: Broadcast ID(254 (0xFE))는 [Ping], [Sync Read] 및 [Bulk Read] 에만 응답하며, 그 외 [Instruction](#instruction)에는 응답하지 않습니다. 예를 들어 [Sync Write] 및 [Bulk Write] Instruction에서는 응답 하지 않습니다.
...
**알림**: Byte Stuffing은 Instruction Packet이 올바르게 처리될 수 있도록 추가 데이터를 삽입하는 방법입니다. Packet에 "0xFF 0xFF 0xFD" 패턴이 나타날 경우, 이 패턴이 다른 Packet의 Header로 해석되지 않도록 0xFD를 추가하여 "0xFF 0xFF 0xFD 0xFD"로 변환합니다.
...
**참고**: Control Table 범위를 초과하여 요청할 경우 Error 필드에 [Access Error](#error)가 채워지고, Parameter가 없는 Status Packet을 응답합니다.
...
|  P1  |              P2 ~ P5              | 설명                                                                                                                                                                                                                  |
|:----:|:---------------------------------:|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0x01 | Fixed value<br />(0x44 0x58 0x4C 0x22) | 현재 위치(Present Position) 값을 1회전(0~4095) 이내의 절대위치 값으로 초기화.<br />정지된 상태에서만 Clear 가능. <br />구동중에 Clear Inst Packet을 전송할 경우, Status Packet의 Error 필드에 Result Fail(0x01) 발생. |
| 0x02 | Fixed value<br />(0x45 0x52 0x43 0x4C) | DYNAMIXEL에 발생한 Error를 Clear.<br />Clear가 불가능한 Error가 있거나 Clear 조건이 아닌 경우 Error는 Clear 되지 않으며 Status Packet의 Error 필드에 Result Fail(0x01) 발생.<br />DYNAMIXEL Y시리즈만 지원합니다.        |
| ...  |                 -                 | 사용하지 않습니다                                                                                                                                                                                                 |
| 0xFF |                 -                 | 사용하지 않습니다                                                                                                                                                                                                 |
...
- Control Table Backup 명령은 Torque Enable 항목이 ‘0’(Off) 상태일 때만 수행되며, ‘1’(On) 상태일 때에는 [Result Fail Packet](#error)을 응답합니다.
...
- RAM 영역의 데이터는 Control Table의 **Startup Configuration(60)** 설정으로 복구할 수 있습니다. 자세한 내용은 [Restoring RAM Area](/docs/software/dynamixel_wizard_2_0/advanced_features#restoring-ram-area)를 참고하세요.
...
|  P1  |              P2 ~ P5              | 설명                                                                                                                 |
|:----:|:---------------------------------:|:---------------------------------------------------------------------------------------------------------------------|
| 0x01 | Fixed value<br />(0x43 0x54 0x52 0x4C) | 현재 상태의 Control Table 데이터를 Backup 영역에 저장                                                                |
| 0x02 | Fixed value<br />(0x43 0x54 0x52 0x4C) | Backup 영역에 저장된 Control Table 데이터 중에서 EEPROM 영역의 데이터를 복구<br />패킷 처리 후 자동으로 재부팅 합니다. |
| 0x03 |                \-                 | 사용하지 않습니다                                                                                                        |
|  ⋯   |                \-                 | 사용하지 않습니다                                                                                                        |
| 0xFF |                \-                 | 사용하지 않습니다                                                                                                        |
...
- [Instruction Packet]에 입력된 ID 순서대로 DYNAMIXEL이 Status Packet 응답합니다.
...
**참고** : 각 장치는 Sync Read Instruction Packet에 대해 응답 시, Status Packet을 각각 반환. 아래 예제를 참고하세요.
...
- Fast Sync Read 패킷을 받은 다수의 DYNAMIXEL은 마치 하나의 DYNAMIXEL이 응답하는 것처럼 [Status Packet]을 하나로 구성하여 응답한다.
...
- Fast Bulk Read 패킷을 받은 다수의 DYNAMIXEL은 마치 하나의 DYNAMIXEL이 응답하는 것처럼 [Status Packet]을 하나로 구성하여 응답한다.
...
[Control Table]: /docs/faq/faq_dynamixel
