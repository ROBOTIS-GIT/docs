...
### Parameters
  ...
  2. 음수 데이터의 표현 방법은 제품별로 차이가 있으므로 해당 제품의 e-Manual을 참고하세요.
...
1. Broadcast ID(254 (0xFE))는 [Ping], [Sync Read] 및 [Bulk Read] 에만 응답하며, 그 외 [Instruction](#instruction)에는 응답하지 않습니다. 예를 들어 [Sync Write] 및 [Bulk Write] Instruction에서는 응답 하지 않습니다.
2. [Control Table]의 Status Return Level 값에 따라 응답 여부가 결정됩니다. 각 장치의 Status Return Level에서 설정 가능한 값을 참고하세요.
...
- Packet ID가 Broadcast ID(0xFE)이고 Option이 Reset all value(0xFF)일 경우, Factory Reset Instruction (0x06)은 동작하지 않습니다
